...
bool DecodeAddress(string str, CService& addr)
{
    ...
    if (!DecodeBase58Check(str.substr(1), vchRet))
        return false;

    struct ircaddr tmp;
    if (vchRet.size() != sizeof(tmp))
        return false;
    memcpy(&tmp, &vchRet[0], sizeof(tmp));
...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("bitcredit-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!ShutdownRequested)
    {
        ...
        if (strMyName == "")
            strMyName = strprintf("x%", GetRand(1000000000));
