...
class CBloomFilter(lib.bitcoin.core.serialize.Serializable):
    ...
    def insert(self, elem):
        ...
        """
        if isinstance(elem, lib.bitcoin.core.COutPoint):
            elem = elem.serialize()
    ...
    def stream_deserialize(cls, f):
        vData = bytearray(lib.bitcoin.core.serialize.BytesSerializer.stream_deserialize(f))
        (nHashFuncs,
         nTweak,
         nFlags) = CBloomFilter.__struct.unpack(lib.bitcoin.core.ser_read(f, CBloomFilter.__struct.size))
    ...
    def stream_serialize(self, f):
        if sys.version > '3':
            lib.bitcoin.core.serialize.BytesSerializer.stream_serialize(self.vData, f)
        else:
            # 2.7 has problems with f.write(bytearray())
            lib.bitcoin.core.serialize.BytesSerializer.stream_serialize(bytes(self.vData), f)
