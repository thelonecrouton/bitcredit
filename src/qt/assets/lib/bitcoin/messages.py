...
# later.
from lib.bitcoin.core import *
from lib.bitcoin.core.serialize import *
from lib.bitcoin.net import *
import bitcoin
...
class MsgSerializable(Serializable):
    ...
    def to_bytes(self):
        ...
        self.msg_ser(f)
        body = f.getvalue()
        res = lib.bitcoin.params.MESSAGE_START
        res += self.command
    ...
    def stream_deserialize(cls, f, protover=PROTO_VERSION):
        ...
        # check magic
        if recvbuf[:4] != lib.bitcoin.params.MESSAGE_START:
            raise ValueError("Invalid message start '%s', expected '%s'" %
                             (b2x(recvbuf[:4]), b2x(lib.bitcoin.params.MESSAGE_START)))
...
class msg_version(MsgSerializable):
    ...
    def __init__(self, protover=PROTO_VERSION):
        ...
        self.strSubVer = (b'/python-bitcoinlib:' +
                          lib.bitcoin.__version__.encode('ascii') + b'/')
