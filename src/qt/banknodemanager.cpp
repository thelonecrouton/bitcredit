...
void BanknodeManager::updateNodeList()
{
	...
	//  -  Crave original  -  QTableWidgetItem *rankItem = new QTableWidgetItem(QString::number(GetMasternodeRank(mn.vin, pindexBest->nHeight)));
	QTableWidgetItem *activeSecondsItem = new QTableWidgetItem(seconds_to_DHMS((qint64)(mn.lastTimeSeen - mn.now)));

	//QString *str1 = boost::lexical_cast<std::string>(mn.lastTimeSeen);
	QTableWidgetItem *lastSeenItem = new QTableWidgetItem(QString::fromStdString(DateTimeStrFormat("%Y-%m-%d %H:%M:%S",mn.lastTimeSeen)));
	//QTableWidgetItem *lastSeenItem = new QTableWidgetItem(seconds_to_DHMS((qint64)(mn.lastTimeSeen))); 
	//QTableWidgetItem *lastSeenItem = new QTableWidgetItem(QString::fromStdString(mn.lastTimeSeen)); 
    ...
    walletModel(0)
{
    ...
    timer = new QTimer(this);
    connect(timer, SIGNAL(timeout()), this, SLOT(updateNodeList()));
    timer->start(30000);            ///////////////////////////////////////////////////////////////    experiment with this     //////////////////////////////
