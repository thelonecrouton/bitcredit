...
const QString apiBittrexTrades = "http://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-BCR&count=100";
const QString apiBittrexOrders = "http://bittrex.com/api/v1.1/public/getorderbook?market=BTC-BCR&type=both&depth=50";
...
void ExchangeBrowser::bittrexMarketSummary(QNetworkReply* response)
{
    ...
    for(int i = 0; i < qslApiResponse.count(); i++){
        ...
        if(read_string(qslApiResponse[i].toStdString(), jsonResponse)) {
            ...
            if (getPairValue(jsonObject, "MarketName").get_str() == "BTC-BCR") {
                try {
