...
void PaperWalletDialog::on_getNewAddress_clicked()
{
    ...
    CBitcreditAddress pubkeyhash;
    pubkeyhash.Set(pubkey.GetID());

    // Create String versions of each
    std::string myPrivKey = CBitcreditSecret(privKey).ToString();
...
void PaperWalletDialog::on_printButton_clicked()
{
   ...
   // Stolen from sendcoinsdialog.cpp
    ...
    if(txFee > 0)
    {
        ...
        questionString.append(BitcreditUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    questionString.append(tr("Total Amount %1 (= %2)")
        .arg(BitcreditUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
